...
def value_of_card(card):
    ...
    if card=='A':
        cardscore=1
    if card in numcards:
        cardscore=int(numcards[int(card)-2])
    return cardscore
...
def can_double_down(card_one, card_two):
    ...
    """
    candouble=False
    doublepoints=[9,10,11]
    card_oneval=value_of_card(card_one)
    card_twoval=value_of_card(card_two)
    if card_twoval+card_oneval in doublepoints:
        candouble=True
