...
def get_rounds(number):
    ...
    """
    cardlist=[]
    for card in range(number, number+3):
...
def average_even_is_average_odd(hand):
    ...
    oddavg=0
    evenavg=0
    for index, card in enumerate(hand[0::2]):
        evensum+=card
        evenavg=evensum/(index+1)
    for index, card in enumerate(hand[1::2]):
        oddsum+=card
        oddavg=oddsum/(index+1)
